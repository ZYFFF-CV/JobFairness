...
def feature_cols(manifest: dict, regime: str) -> list[dict]:
    ...
    if manifest["numeric_features"]:
        cols.append(
            {
                "name": manifest["numeric_features"],
                "active": True,
                "dtype": "float",
                "type": "numeric",
...
def make_entry(manifest: dict, processed_dir: Path, data_root: str, regime: str, mode: str) -> dict:
    ...
    if mode == "smoke":
        ...
        train_data = processed_dir / "train_full.csv"
    ...
    return {
        ...
        "min_categr_count": 1,
        "feature_cols": feature_cols(manifest, regime),
...
def main() -> None:
    ...
    config = {}
    # Generate every supported protocol combination from the same manifest. This
    # avoids hand-written feature lists and makes the protected-feature delta
    # between unaware and unfair easy to audit.
